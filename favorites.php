...
$title = $conf['website_name_short'] . ' - ' . $lang['Favorites'];

include PATH . '/templates/' . $cookie_template . '/header.php';

$tpl = PATH . '/templates/' . $cookie_template . '/tpl/favorites_header.tpl';
$template = new Template;
$template->load ( $tpl );

$template->set( 'list_text', $list_text );

$template->set( 'header', $lang['Favorites']  );

$template->publish();

// Grab their favorite listings
...
// See if they have any favorite listings, otherwise output an error
if ( isset( $_COOKIE['favoriteListings'] ) )
{
	if ( strstr( $_COOKIE['favoriteListings'], ';' ) )
	{
		$listing_ids = str_replace( ';', ', ', $_COOKIE['favoriteListings'] );
		$listing_ids = rtrim( $listing_ids, ', ' );
	}
	else
	{
		$listing_ids = $_COOKIE['favoriteListings'];
	}
}
else
{
...
if ( $db->numrows( $q ) > 0 )
{
	...
	{
		$tpl = PATH . '/templates/' . $cookie_template . '/tpl/favorites.tpl';
		$template = new Template;
		$template->load( $tpl );
...
$tpl = PATH . '/templates/' . $cookie_template . '/tpl/favorites_footer.tpl';
$template = new Template;
$template->load ( $tpl );
...
$custom['pagination'] = pagination( URL . '/favorites.php', $_REQUEST['page'], $total_results, $conf['search_results'] );
