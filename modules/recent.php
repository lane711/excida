...
$tpl = PATH . '/templates/' . $cookie_template . '/tpl/recent_listings_header.tpl';
$template = new Template;
$template->load( $tpl );
$template->set( '@recent_properties', $lang['Module_Recent_Listings'] );
$template->publish();
...
// Fetch most recent approved listings
$sql = "
SELECT
	p.*,
...
if ( $db->numrows( $q ) > 0 )
{
	...
	{
		$tpl = PATH . '/templates/' . $cookie_template . '/tpl/recent_listings.tpl';
		$template = new Template;
		$template->load( $tpl );
...
// Load header of this template
$tpl = PATH . '/templates/' . $cookie_template . '/tpl/recent_listings_footer.tpl';
$template = new Template;
$template->load( $tpl );
$template->publish();
