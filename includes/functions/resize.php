...
if(! defined('FILE_CACHE_SUFFIX') ) 		define ('FILE_CACHE_SUFFIX', '.resize.txt');			// What to put at the end of all files in the cache directory so we can identify them
if(! defined('FILE_CACHE_PREFIX') ) 		define ('FILE_CACHE_PREFIX', 'resize');				// What to put at the beg of all files in the cache directory so we can identify them
if(! defined('FILE_CACHE_DIRECTORY') ) 		define ('FILE_CACHE_DIRECTORY', '../../media/cache');	// Directory where images are cached. Left blank it will use the system temporary directory (which is better for security)
if(! defined('MAX_FILE_SIZE') )				define ('MAX_FILE_SIZE', 30485760);						// 10 Megs is 10485760. This is the max internal or external file size that we'll process.  
...
if(! defined('NOT_FOUND_IMAGE') )			define ('NOT_FOUND_IMAGE', '../../media/error.png');							// Image to serve if any 404 occurs 
if(! defined('ERROR_IMAGE') )				define ('ERROR_IMAGE', '../../media/error.png');								// Image to serve if an error occurs instead of showing error message 
...
if(! defined('DEFAULT_ZC') )				define ('DEFAULT_ZC', 2);								// Default zoom/crop setting. Allows overrid in timthumb-config.php
